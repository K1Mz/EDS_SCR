...
//Department
  function proSel()
  {
    ...
    $proq=$this->db->get();
    return $proq->result_array();
  }
  function editDepDB($use_id)
  {
...
//User
  ...
  function listdep()
  {
    ...
    $listq = $this->db->get();
    $option = array();
    $option[0] = 'PLEASE SELECT DEPARTMENT'; 
  ...
  function proEditSel($use_id)
  {
    $this->db->select('use_id, use_user, use_pass, use_fname, use_lname, use_email, use_status, dep_name');
    $this->db->from('user u');
    $this->db->join('department d','d.dep_id=u.use_dep_id');
